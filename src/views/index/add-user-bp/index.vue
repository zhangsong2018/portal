<template>
  <div id="wrap">
       ...
       <div class="section" v-if="personList.length>0">
          ...
          <div class="filter">
            <p @click="show1 = !show1" style="margin-left: 50px;font-size: 14px;display: flex;align-items: center;">
          ...
          <div class="adduser_list">
             <div class="userlist_item" :class="{'active':item.priority=='1'}" :style="{'background':item.longPressActive?'#eeeeee':''}" v-for="item in Newitems" @touchstart="gtouchstart(item)" @touchmove="gtouchmove()" @touchend="gtouchend(item)">
               <p class="usernum">{{item.bedCode}}</p>
               <p class="username">{{item.name}}</p>
               <p class="usersex"><span v-if="item.gender == 'Male'"><img src="@/static/images/nan.png" style="transform:scale(0.5);margin-top:5px"></span><span v-if="item.gender != 'Male'"><img src="@/static/images/nv.png" style="transform:scale(0.5);margin-top:5px"></span></p>
       ...
       <div class="section" v-if="personList.length==0 && !sectionShow">
          <div class="adduser_list">
              <img src="@/static/images/sysIcon.png" style="transform:translateX(-50%);margin-left: 50%;width: 120px;height: 120px;margin-top:150px;">
...
<script>
import { PopupHeader, Popup,Radio ,Icon,Toast, Group, XSwitch, XButton ,Loading,TransferDomDirective as TransferDom} from 'vux'
import Vue from 'vue'
                ...
                if(_this.value == '全部' && _this.valuenum == '全部'){
                   ...
                   data.result.items.map(function(item) {
                     if (item.abilityLevel.search(_this.value) != -1 && _this.valuenum == '全部') {
                        Newitem.push(item);
                     }else if(item.bedCode.substring(0,1).search(_this.valuenum) != -1 && _this.value == '全部'){
                        Newitem.push(item);
                     }else if(item.abilityLevel.search(_this.value) != -1 && item.bedCode.substring(0,1).search(_this.valuenum) != -1){
                        Newitem.push(item);
                     }
                   });
                   if(Newitem.length == 0){
