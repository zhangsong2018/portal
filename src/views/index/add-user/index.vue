<template>
  <div id="wrap">
    ...
    <div class="section" v-if="personList.length>0">
      ...
      <div class="filter">
        <p @click="show1 = !show1" style="margin-left: 50px;font-size: 14px;align-items: center;display: flex;">
      ...
      <div class="adduser_list">
        <div class="userlist_item" :class="{'active':item.priority=='1'}"
             :style="{'background':item.longPressActive?'#eeeeee':''}" v-for="item in Newitems"
             @touchstart="gtouchstart(item)" @touchmove="gtouchmove()" @touchend="gtouchend(item)">
          <p class="usernum">{{item.bedCode}}</p>
          <p class="username">{{item.name}}</p>
          <p class="usersex"><span v-if="item.gender == 'Male'"><img src="@/static/images/nan.png"
                                                                     style="transform:scale(0.5);margin-top:5px"></span><span
            v-if="item.gender != 'Male'"><img src="@/static/images/nv.png" style="transform:scale(0.5);margin-top:5px"></span>
          </p>
    ...
    <div class="section" v-if="personList.length==0 && !sectionShow">
      <div class="adduser_list">
        <img src="@/static/images/sysIcon.png"
             style="transform:translateX(-50%);margin-left: 50%;width: 120px;height: 120px;margin-top:150px;">
    ...
    <ul class="lists" v-if="listShowMenu">
      <li class="list_item" v-for="item in tenants" @click="tabMenu(item)">{{item.name}}</li>
    </ul>
  ...
  export default {
    ...
    data() {
      return {
        ...
        cancel: '重置',
        TenantId: '',
        options: ['全部', '基础服务', '照护一级', '照护二级A', '照护二级B', '照护三级A', '照护三级B', '照护三级B（失智）'],
        optionsnum: ['全部', '01', '02', '03', '05', '06', '07', '08', '09', '10', '11', '12', '13']
    ...
    methods: {
            ...
            if (data.success) {
              if (_this.value == '全部' && _this.valuenum == '全部') {
                ...
                data.result.items.map(function (item) {
                  if (item.bedCode.length > 5) {
                    if (item.abilityLevel.search(_this.value) != -1 && _this.valuenum == '全部') {
                      Newitem.push(item);
                    } else if (item.bedCode.substring(0, 2).search(_this.valuenum) != -1 && _this.value == '全部') {
                      Newitem.push(item);
                    } else if (item.abilityLevel.search(_this.value) != -1 && item.bedCode.substring(0, 2).search(_this.valuenum) != -1) {
                      Newitem.push(item);
                    }
                  } else {
                    item.bedCode1 = '0' + item.bedCode;
                    if (item.abilityLevel.search(_this.value) != -1 && _this.valuenum == '全部') {
                      Newitem.push(item);
                    } else if (item.bedCode1.substring(0, 2).search(_this.valuenum) != -1 && _this.value == '全部') {
                      Newitem.push(item);
                    } else if (item.abilityLevel.search(_this.value) != -1 && item.bedCode1.substring(0, 2).search(_this.valuenum) != -1) {
                      Newitem.push(item);
                    }
                  }
      ...
      setTopTop(item) {
        ...
        function ajaxSend() {
          $.ajax({
            type: "post",    //请求方式
            url: url + "/api/services/app/OmsCareService/MoSetCareRelationPriority",
            dataType: "json",    //跨域json请求一定是jsonp
            timeout: 10000,
            headers: {
              'Authorization': 'Bearer ' + Authorization,
      ...
      init() {
        this.TenantId = localStorage.getItem("tenantId") ? localStorage.getItem("tenantId") : ''
        var _this = this;
        var url = this.url;
              ...
              $.ajax({
                ...
                success: function (data) {
                  ...
                  _this.personList.forEach(function (v, i) {
                    if (v.id == data.result.careRelationId || v.careRelationId == data.result.careRelationId) {
                      _this.position = 'middle';
                      _this.show = true;
                      _this.conText = "用户已存在！";
                      _this.show2 = false;
                  ...
                  if (data.result.customerName == "" || data.result.customerName == null) {
                    _this.position = 'middle';
                    _this.show = true;
      ...
      addUser() {
        var storage = window.localStorage;
        storage.setItem("userInfo", JSON.stringify(this.personList));
        this.show = true;
        this.conText = "保存成功";
        var userInfo = JSON.parse(storage.getItem("userInfo"));
        this.rid = userInfo[0].residentId;
        //this.$router.push('/CarePlan');
        this.$router.push({
          path: 'CarePlan',
          name: 'CarePlan',
          params: {
