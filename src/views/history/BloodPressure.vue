<template>
  <div class="wrap" v-title="title">
    ...
    <HeaderBarIT><span slot="title">血压历史</span></HeaderBarIT>
    <!--滑动区域-->
    <section>
      <div id="mescroll" class="mescroll_top"><!--v-show="items.length>0"-->
        <ul id="dataList" class="data-list">
          <li v-for="item in items" :key="item.id" class='lists' v-if="item.item ==8">
            <span class="flex_item" style="flex: 2">{{item.obsTime | dateFormat}}</span>
            <span class="flex_item" style="flex: 2">
                   <span v-if='item.value!= "" && item.value2!= ""'>{{item.value}}/{{item.value2}} mmHg</span>
                 </span>
            <span class="flex_item" v-if="item.value3 == 1">拒测</span>
            <span class="flex_item" v-if="item.value3 != 1"></span>
          </li>
          <li v-for="item in items" :key="item.id" class='lists' v-if="item.item ==2">
            <span class="flex_item">{{item.obsTime | dateFormat}}</span>
            <span class="flex_item" v-if="item.value != ''"><span>{{item.value}}次 / 分</span></span>
            <span class="flex_item" v-if="item.value == ''">拒测</span>
          </li>
        </ul>
      </div>
      <!--<div class="noData">&lt;!&ndash;v-show="items.length==0"&ndash;&gt;-->
